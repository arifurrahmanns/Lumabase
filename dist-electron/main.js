...
function getPidByPort(port) {
  try {
    if (process.platform === "win32") {
      const output = require$$0$3.execSync(`netstat -ano | findstr :${port}`).toString();
      ...
      const output = require$$0$3.execSync(`lsof -i :${port} -t`).toString();
...
class EngineController {
  ...
  async addInstance(instance, onProgress) {
    ...
    if (!fs$1.existsSync(instance.binaryPath)) {
      ...
      try {
        ...
        const installDir = path$1.dirname(instance.dataDir);
  ...
  removeInstance(id) {
    ...
    if (instance) {
      ...
      if (instance.dataDir && fs$1.existsSync(instance.dataDir)) {
        try {
          console.log(`Removing data directory: ${instance.dataDir}`);
          fs$1.rmSync(instance.dataDir, { recursive: true, force: true });
        } catch (e) {
          console.error(`Failed to remove data dir: ${e}`);
        }
      }
      if (instance.dataDir) {
        const versionDir = path$1.dirname(instance.dataDir);
        const instanceDir = path$1.dirname(versionDir);
        if (fs$1.existsSync(instanceDir)) {
          try {
            console.log(`Removing instance directory: ${instanceDir}`);
            fs$1.rmSync(instanceDir, { recursive: true, force: true });
          } catch (e) {
  ...
  async startInstance(id) {
    ...
    try {
      ...
      if (instance.type === "mysql") {
        ...
        if (!fs$1.existsSync(instance.dataDir)) {
          fs$1.mkdirSync(instance.dataDir, { recursive: true });
        }
        const mysqlDir = path$1.join(instance.dataDir, "mysql");
        if (!fs$1.existsSync(mysqlDir)) {
          console.log(`Initializing MySQL data directory: ${instance.dataDir}`);
          try {
            require$$0$3.execSync(`"${instance.binaryPath}" --initialize-insecure --datadir="${instance.dataDir}"`);
          } catch (e) {
        ...
        child = require$$0$3.spawn(instance.binaryPath, args);
        ...
        const pgVersionFile = path$1.join(instance.dataDir, "PG_VERSION");
        if (!fs$1.existsSync(pgVersionFile)) {
          console.log(`Initializing Postgres data directory: ${instance.dataDir}`);
          try {
            const binDir = path$1.dirname(instance.binaryPath);
            const initdbPath = path$1.join(binDir, process.platform === "win32" ? "initdb.exe" : "initdb");
            if (!fs$1.existsSync(initdbPath)) {
              throw new Error(`initdb not found at ${initdbPath}. Cannot initialize.`);
            }
            require$$0$3.execSync(`"${initdbPath}" -D "${instance.dataDir}" -U postgres --auth=trust`);
          } catch (e) {
...
function createWindow() {
  win = new electron.BrowserWindow({
    ...
    // Hide title bar but keep traffic lights on macOS if needed, though frame: false usually hides everything on Windows
  });
...
electron.app.on("before-quit", async (_event) => {
