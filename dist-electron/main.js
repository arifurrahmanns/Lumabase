...
class MysqlAdapter {
  ...
  async connect(config2) {
    try {
      ...
      this.connection = await mysql.createConnection({
        host: config2.host,
        port: parseInt(config2.port) || 3306,
        user: config2.user,
  ...
  async deleteRows(tableName, primaryKeyColumn, primaryKeyValues) {
    if (!this.connection) throw new Error("Database not connected");
  ...
  async updateRows(tableName, updateCol, updateVal, pkCol, pkVals) {
    if (!this.connection) throw new Error("Database not connected");
  ...
  async listUsers() {
    ...
    return rows.map((row) => ({ username: row.User, host: row.Host }));
  }
...
class PostgresAdapter {
  ...
  async listUsers() {
    ...
    return res.rows.map((row) => ({ username: row.usename, host: "%" }));
  }
  async createUser(user) {
    if (!this.client) throw new Error("Database not connected");
...
class DatabaseManager {
  ...
  async cloneConnection(connectionId, newDbName) {
    try {
      ...
      if (!config2) throw new Error("Could not retrieve config from adapter");
      const newConfig = { ...config2, database: newDbName };
  ...
  async deleteRows(connectionId, tableName, pkCol, pkVals) {
    const adapter = this.getAdapter(connectionId);
    if (adapter.deleteRows) {
      return adapter.deleteRows(tableName, pkCol, pkVals);
    }
  ...
  async updateRows(connectionId, tableName, updateCol, updateVal, pkCol, pkVals) {
    const adapter = this.getAdapter(connectionId);
    if (adapter.updateRows) {
      return adapter.updateRows(tableName, updateCol, updateVal, pkCol, pkVals);
    }
    for (const val of pkVals) {
      const row = {};
...
class EngineConfigManager {
  ...
  saveConfig(config2) {
    try {
      console.log("Saving config:", JSON.stringify(config2, null, 2));
  ...
  getInstances() {
    ...
    return config2.instances;
  }
  addInstance(instance) {
    const config2 = this.readConfig();
    config2.instances.push(instance);
    this.saveConfig(config2);
  }
  removeInstance(id) {
    const config2 = this.readConfig();
    config2.instances = config2.instances.filter((i) => i.id !== id);
    this.saveConfig(config2);
  }
  updateInstance(id, updates) {
    const config2 = this.readConfig();
    const index = config2.instances.findIndex((i) => i.id === id);
    if (index !== -1) {
      config2.instances[index] = { ...config2.instances[index], ...updates };
      this.saveConfig(config2);
    }
  }
}
...
const http2Transport = {
  request(options, cb) {
    ...
    utils$2.forEach(headers2, (header, name) => {
...
const downloadEngine = async (type2, version2, destDir, onProgress) => {
  const key = `${type2}-${version2}`;
  const url2 = DOWNLOAD_URLS[key];
  if (!url2) {
    throw new Error(`No download URL for ${type2} ${version2}`);
  }
  const tempDir = path$1.join(electron.app.getPath("temp"), "lumabase-downloads");
  if (!fs$1.existsSync(tempDir)) {
    fs$1.mkdirSync(tempDir, { recursive: true });
  }
  const fileName = path$1.basename(url2);
  const filePath = path$1.join(tempDir, fileName);
  if (fs$1.existsSync(destDir)) {
    try {
      const subdirs2 = fs$1.readdirSync(destDir).filter((f) => {
        return fs$1.statSync(path$1.join(destDir, f)).isDirectory() && f !== "data";
      });
      const rootExtractDir2 = subdirs2.length === 1 ? path$1.join(destDir, subdirs2[0]) : destDir;
      let checkBin = "";
      if (type2 === "mysql") {
        checkBin = path$1.join(rootExtractDir2, "bin", "mysqld.exe");
      } else {
        checkBin = path$1.join(rootExtractDir2, "bin", "postgres.exe");
      }
      if (fs$1.existsSync(checkBin)) {
  ...
  console.log(`Downloading ${key} from ${url2}...`);
  if (!fs$1.existsSync(filePath)) {
    ...
    fs$1.writeFileSync(filePath, response.data);
  } else {
  ...
  const subdirs = fs$1.readdirSync(destDir).filter((f) => {
    return fs$1.statSync(path$1.join(destDir, f)).isDirectory() && f !== "data";
  });
  const rootExtractDir = subdirs.length === 1 ? path$1.join(destDir, subdirs[0]) : destDir;
  let binPath = "";
  if (type2 === "mysql") {
    binPath = path$1.join(rootExtractDir, "bin", "mysqld.exe");
  } else {
    binPath = path$1.join(rootExtractDir, "bin", "postgres.exe");
  }
  if (!fs$1.existsSync(binPath)) {
    const altBin = path$1.join(destDir, "bin", type2 === "mysql" ? "mysqld.exe" : "postgres.exe");
    if (fs$1.existsSync(altBin)) binPath = altBin;
...
function getPidByPort(port) {
  try {
    if (process.platform === "win32") {
      const output = require$$0$1.execSync(`netstat -ano | findstr :${port}`, { encoding: "utf-8" });
      ...
      try {
        const output = require$$0$1.execSync(`lsof -i :${port} -t`, { encoding: "utf-8" });
...
class EngineController extends events$1.EventEmitter {
  constructor() {
    super();
    __publicField(this, "instances");
    __publicField(this, "runningProcesses");
  ...
  async addInstance(instance, onProgress) {
  ...
  removeInstance(id) {
    ...
    if (instance) {
      ...
      if (instance.dataDir && fs__namespace.existsSync(instance.dataDir)) {
        try {
          console.log(`Removing data directory: ${instance.dataDir}`);
          fs__namespace.rmSync(instance.dataDir, { recursive: true, force: true });
        } catch (e) {
          console.error(`Failed to remove data dir: ${e}`);
        }
      }
      if (instance.dataDir) {
        const versionDir = path__namespace.dirname(instance.dataDir);
        const instanceDir = path__namespace.dirname(versionDir);
        if (fs__namespace.existsSync(instanceDir)) {
          try {
            console.log(`Removing instance directory: ${instanceDir}`);
            fs__namespace.rmSync(instanceDir, { recursive: true, force: true });
          } catch (e) {
  ...
  async startInstance(id) {
    ...
    try {
      ...
      if (instance.type === "mysql") {
        ...
        const mysqlDir = path__namespace.join(instance.dataDir, "mysql");
        if (!fs__namespace.existsSync(mysqlDir)) {
          console.log(`Initializing MySQL data directory: ${instance.dataDir}`);
          try {
            require$$0$1.execSync(`"${instance.binaryPath}" --initialize-insecure --datadir="${instance.dataDir}"`);
          } catch (e) {
        ...
        child = require$$0$1.spawn(instance.binaryPath, args);
  ...
  async stopInstance(id) {
    ...
    let pidToKill = child == null ? void 0 : child.pid;
    if (!pidToKill && (instance == null ? void 0 : instance.pid)) {
  ...
  updateStatus(id, status, pid, lastStartedAt) {
    const instance = this.instances.get(id);
    if (instance) {
...
function createWindow() {
  win = new electron.BrowserWindow({
    ...
    // Hide title bar but keep traffic lights on macOS if needed
  });
...
electron.app.whenReady().then(() => {
  createWindow();
  electron.ipcMain.handle("connect-database", async (_, config2) => {
    return dbManager.connect(config2);
  });
  electron.ipcMain.handle("clone-connection", async (_, { connectionId, newDbName }) => {
  ...
  electron.ipcMain.handle("update-rows", async (_, { connectionId, tableName, updateCol, updateVal, primaryKeyColumn, primaryKeyValues }) => {
  ...
  electron.ipcMain.handle("engine-create", async (event, instance) => {
    return engineController.addInstance(instance, (percent) => {
      event.sender.send("engine-download-progress", percent);
    });
  });
  electron.ipcMain.handle("engine-remove", async (_event, id) => {
    return engineController.removeInstance(id);
  });
  electron.ipcMain.handle("engine-start", async (_event, id) => {
    return engineController.startInstance(id);
  });
  electron.ipcMain.handle("engine-stop", async (_event, id) => {
    return engineController.stopInstance(id);
  });
  electron.ipcMain.handle("engine-update", async (_event, { id, updates }) => {
    return engineController.updateInstanceConfig(id, updates);
  });
  electron.ipcMain.handle("get-default-engine-paths", async () => {
    const userDataPath = electron.app.getPath("userData");
    const enginesPath = path$2.join(userDataPath, "engines");
    return {
      base: enginesPath,
  ...
  const iconPath = path$2.join(process.env.VITE_PUBLIC || "", "app-icon.png");
  const icon = electron.nativeImage.createFromPath(iconPath);
  const updateTrayMenu = (instances) => {
    if (!tray) return;
    const template = [
      {
        label: "Open",
  ...
  electron.ipcMain.handle("get-app-settings", () => {
    return {
      startOnLogin: electron.app.getLoginItemSettings().openAtLogin,
      showInTaskbar: true,
      showInTray: !!tray
    };
  });
  electron.ipcMain.handle("set-start-on-login", (_, openAtLogin) => {
